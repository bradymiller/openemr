...
if ($_REQUEST['REFTYPE']) {
    ...
    $query = "SELECT * FROM form_eye_mag JOIN forms 
    on forms.form_id = form_eye_mag.id 
    ...
    if ($REFTYPE =="W") {
        ...
        if ($data['RX1']=='0') {
            ...
            while ($row = sqlFetchArray($CTLMANUFACTURER_data)) {
              if ($data['CTLMANUFACTUREROD'] == $row['option_id']) $CTLMANUFACTUREROD = $row['title'];
              if ($data['CTLMANUFACTUREROS'] == $row['option_id']) $CTLMANUFACTUREROS = $row['title'];
            }
            ...
            while ($row = sqlFetchArray($CTLSUPPLIER_data)) {
              if ($data['CTLSUPPLIEROD'] == $row['option_id']) $CTLSUPPLIEROD = $row['title'];
              if ($data['CTLSUPPLIEROS'] == $row['option_id']) $CTLSUPPLIEROS = $row['title'];
            }
            ...
            while ($row = sqlFetchArray($CTLBRAND_data)) {
              if ($data['CTLBRANDOD'] == $row['option_id']) $CTLBRANDOD = $row['title'];
              if ($data['CTLBRANDOS'] == $row['option_id']) $CTLBRANDOS = $row['title'];
            }
    }
            ...
            <div style="margin:5;text-align:center;">
                <table> 
                    ...
                    if (sqlNumRows($dispensed) == 0) {
                        echo "<tr><td colspan='2' style='text-align:middle;padding:25px;'>".xlt('There are no Glasses or Contact Lens Presciptions on file for this patient')."</td></tr>";
                    }
                    ?>
                </table>
                <?php
                while ($row = sqlFetchArray($dispensed)) {
                    ...
                    ?>
                    <div id="RXID_<?php echo $row['id']; ?>" style="position:relative;width:440px;text-align:center;margin: 10 auto;">
                        <i class="pull-right fa fa-close" onclick="delete_me('<?php echo $row['id']; ?>');" title="<?php echo attr('Remove this Prescription from the list of RXs dispensed'); ?>"></i>
                        <table>
                        <tr>
                                ...
                                if ($row['REFTYPE'] != "CTL") { ?>
                                    <table id="SpectacleRx" name="SpectacleRx" class="refraction">
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo attr($row['ODSPH']); ?></td>
                                            <td><?php echo attr($row['ODCYL']); ?></td>
                                            <td><?php echo attr($row['ODAXIS']); ?></td>
                                            <td><?php echo attr($row['ODPRISM']); ?></td>
                                        </tr>
                                        <tr>
                                            <td><b><?php echo xlt('OS'); ?></b></td>
                                            <td><?php echo attr($row['OSSPH']); ?></td>
                                            <td><?php echo attr($row['OSCYL']); ?></td>
                                            <td><?php echo attr($row['OSAXIS']); ?></td>
                                            <td><?php echo attr($row['OSPRISM']); ?></td>
                                        </tr>
                                        <tr class="NEAR">
                                            ...
                                            <td><b><?php echo xlt('OD'); ?></b></td>
                                            <td class="WMid"><?php echo attr($row['ODADD1']); ?></td>
                                            <td class="WAdd2"><?php echo attr($row['ODADD2']); ?></td>
                                        </tr>
                                        <tr class="NEAR">
                                            <td><b><?php echo xlt('OS'); ?></b></td>
                                            <td class="WMid"><?php echo attr($row['OSADD1']); ?></td>
                                            <td class="WAdd2"><?php echo attr($row['OSADD2']); ?></td>
                                        </tr>
                                        <tr style="">
                                            ...
                                            <!-- <td> 
                                                ...
                                                <span href="print.php?RXTYPE=W" class="ui-icon ui-icon-cancel" onclick= "window.print();" Xonclick="top.restoreSession(); window.print(); return false;" style="display:inline-block"><?php echo xlt("Print"); ?></span> 
                                            </td> -->
                                        </tr>
                                    </table>
                                    <?php 
        ...
        <body>
            ...
            <form method="post" action="<?php echo $rootdir;?>/forms/<?php echo text($form_folder); ?>/SpectacleRx.php?mode=update" id="Spectacle" class="eye_mag pure-form" name="Spectacle" style="text-align:center;">
              <!-- start container for the main body of the form -->
                <input type="hidden" name="REFDATE" id="REFDATE" value="<?php echo $data['date']; ?>">
