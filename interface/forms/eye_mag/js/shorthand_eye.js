...
function process_kb_1(field,text,appendix) {
    ...
    var  prior_text;
        ...
        // Need regexp to distinguish "." to make this 3 entries to MEDS.
            //note we use periods in drug concs so that cannot be the delimiter...?
            //What if we start segment with 5-MP as the drug?
            //what about logic to say delimiter cannot be preceeded by and followed by a non-digit?
            //Do we use the full-stop anywhere else in a drug list???
        //ALL:Sulfa rash.PCN hives;  2 added with reaction to Allergies.
        //
        //do a foreach on text separated by "."
        //POH.Phaco/IOL OD 4/4/1994.Phaco/IOL OS 4/24/1995.
        //match and remove date?  Not yet. In time.  FEATURE REQUEST.
        //upload to server.
    
        //0=PMH,1=Allergy,2=Meds,3=surgery,4=Dental,5=POH,6=FH,7=SH,8=ROS
        
        // First list fields that exist but need special processing...
        // this method should work for PMH,ALLERGY,MEDS,SURGERY,POH (<--openEMR terms).
            //Maybe Dental but we are not using that so leave it to others.
            //Not FH,SH or ROS though..
            //Possible abbreviations for these fields:
    ...
    if ((field =="PMH")||
        ...
        (field =="Surgery")||
        (field =="POH")) {
        //if you create your own Issue, do need to identify it now?
        //do a foreach on text separated by "."
        //var text_seg = text.replace(/^[\s]*/,'').match(/([^\d]\.[^\d]*)/g);
        //need a reg exp to distinguish 0.5% from trailing \.$ as a segment end.
        //try replacing \d\.\d with \dUGLYHACK\d
        //then replace back after segmented out.
        //This is where a real programmer would be nice.
        ...
        for (index=0; index < text_seg.length; ++index) {
            ...
            text_seg[index] = text_seg[index].replace(/UGLYHACK/g,".");
                // straight upload for PMH,MEDS,SURGERY,POH.
                // ALLERGY: DRUG-space-reaction? OK
                // Suck out date from these as onset date? maybe later
                  //upload text_seg[index] to issue
            ...
            .always(function() { //alert("complete "+text_seg[index]);
                    });
        }
