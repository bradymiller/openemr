...
var Code_new_est="Est";//options are New and anything else defaults to Established Codes
 ...
 */
function fill_QP_field(PEZONE, ODOSOU, LOCATION_text, selection,fill_action,Code_to_process) {
    if (ODOSOU > '') {
...
function alter_issue2(issue_number,issue_type,index) {
    ...
    var location = $("#PMH_anchor").offset().top -55;
...
/* START Functions related to IMPPLAN Builder */
    ...
    function build_DX_list(obj) {
        ...
        if ($('#inc_PMH').is(':checked') && obj.PMSFH['PMH']) {
         ...
         if (out !="") {
            ...
            out = '<br /><span class="bold">Build Your Plan.</span><br /><br>';
            out += 'The Builder incorporates items from the checked areas:  Exam, Past Ocular History (POH) and Past Medical History (PMH).<br />';
            $( "#build_DX_list" ).html(out);
        }
    }
    /**
    ...
    function build_IMPPLAN(items) {
        ...
        if ((items == null) || ((typeof items == "undefined")|| (items.length =='0'))) {
            ...
            $.each(items, function( index, value ) {
              ...
              if ((value.code=="") || (value.code.match(/Code/))) {
                  ...
                  if ((typeof value.PMSFH_link !== "undefined") || (value.PMSFH_link == null)) {
                      ...
                      for (i=0;i < CodeArr.length;i++) {
                          if (CodeArr.length == (TitleArr.length-1)) { //there is a trailing \r
                            ...
                            $('#Coding_DX_Codes').append(CodeArr[i]+': Manually retrieve description on Fee Sheet   <br />');
                          }
                      }
                  } else  {
              ...
              value.title +"</span>"+
              "<span contenteditable class='pull-right' onclick='sel_diagnosis("+index+",\""+value.title+"\");' title='"+value.codetext+"' id='CODE_"+index+"'>"+
              value.code + "</span>&nbsp;"+
    ...
    function store_IMPPLAN(storage) {
        if (typeof storage !== "undefined") {
            var url = "../../forms/eye_mag/save.php?mode=update&YackThack";
               ...
               data 		: {
                   ...
                   uniqueID      : $('#uniqueID').val()
                       ...
                       if (result == "Code 400") {
                        ...
                        alert("Voila! Check out the fee sheet to process today's charges.");
                       }
                       });
    }
...
function Suggest_visit_code() {
    ...
    $('#visit_codes').val(default_search_type+"|"+visit_code+"|").change();
}
/*
...
function build_CODING_list() {
    ...
    */
        //1.  Visit Codes.
    code_type = default_search_type;
    CODING_items.push({
                      code:     visit_code,
                      codedesc: visit_desc,
                      codetext: '',
                      codetype: default_search_type,
                      title:    'Visit Code'
                      });
        //neurosensory
    if (CPT_92060 == 'here') {
...
$(document).ready(function() {
                  ...
                  $("[id$='_keyboard'],[id$='_keyboard_left']").on('keydown', function(e) {
                                                                   ...
                                                                   for (index=0; index < data_seg.length; ++index) {
                                                                       ...
                                                                       if ((index =='0') && (data_seg[index].match(/^D($|;)/))) {
                                                                   alert("Default");
                  ...
                  $('#BUTTON_PRINT_report_2').click(function() {
                                                    store_PDF();
                                                    });
