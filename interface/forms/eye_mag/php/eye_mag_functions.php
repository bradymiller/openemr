...
function first_run() {
    ...
    // If Fundus is not here, then this function is run.

    $query = "select id from categories where name = 'Fundus'";
    $result = sqlStatement($query);
    $FUNDUS = sqlFetchArray($result);
    $fundus = $FUNDUS['id'];

    if ($fundus < '1') {
        $query = "select id from categories where name = 'Medical Record'";
        $result = sqlStatement($query);
        $MRID = sqlFetchArray($result);
        $medical_record = $MRID['id'];

        $query = "select id from categories where name = 'Imaging' and parent=?";
        $result = sqlStatement($query,array($medical_record));
        $IMAGING = sqlFetchArray($result);
        $imaging = $IMAGING['id'];
       ...
       $queries = "INSERT INTO categories select (select MAX(id) from categories) + 1,'External Photos','EXT','".$imaging."',rght, rght + 1 from categories where name = 'Imaging' and parent='".$medical_record."';
            ...
            mysql_multiquery($queries); //I know, I know...  Scary but effective.
        
    /* These lines can probably be deleted.  Testing with them commented out...
        

        They remain here for now so we know what to add to the upgrade script if they are not already there...
