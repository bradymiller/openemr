    ...
    if ($zone == "EXT") {
                ...
                <div id="PRIOR_NPCNPA" name="PRIOR_NPCNPA">
                    <table style="position:relative;float:left;text-align:center;margin: 4 2;width:100%;font-size:1.0em;padding:4px;">
                        <tr style="">
                            ...
                            <td style="font-weight:bold;"><?php echo xlt('OD'); ?></td>
                            <td style="font-weight:bold;"><?php echo xlt('OS'); ?></td>
                        </tr>
                        <tr>
        ...
        if ($PRIOR_IMPPLAN_items) { 
            ...
            foreach ($PRIOR_IMPPLAN_items as $item) {
                ...
                echo "  <span>$k. ".text($item['title'])."</span><span class='pull-right'>".text($item['code'])."</span><br />";
...
function build_PMSFH($pid) {
    ...
    */
    ...
    // just like we are doing for Impression Plan.  Mybe we can piggybak onto one of the ROS table already in OpenEMR?
...
function show_PMSFH_panel($PMSFH,$columns='1') {
    ...
    //<!-- POH -->
    echo "<br /><span class='panel_title' title='".xla('Past Ocular History')."'>".xlt("POH").":</span>";
    ?>
    ...
    //<!-- POS -->
    echo "<br /><span class='panel_title' title='".xla('Past Ocular Surgery')."'>".xlt("POS").":</span>";
    ?>
    <span class="top-right btn-sm" href="#PMH_anchor" 
    ...
    //<!-- PMH -->
    echo "<br /> <span class='panel_title' title='".xla('Past Medical History')."'>".xlt("PMH").":</span>";
    ?><span class="top-right btn-sm" href="#PMH_anchor" 
    ...
    if ($PMSFH[0]['Allergy'] > "") {
      ...
      $i++;
       ...
       //<!-- Social History -->
    echo "<br /><span class='panel_title' title='".xlt("Social History")."'>Soc Hx:</span>";
    ?><span class="top-right btn-sm" href="#PMH_anchor" 
...
function copy_forward($zone,$copy_from,$copy_to,$pid) {
    ...
    if ($zone =="EXT") {
        ...
        $count_rx++;
        if ($count_rx < 5) {
            for ($i = $count_rx; $i < 5; $i++) {
            $result["display_W_$i"] = '';
            $result["ODSPH_$i"] = $wearing['ODSPH'];
            $result["ODCYL_$i"] = $wearing['ODCYL'];
            $result["ODAXIS_$i"] = $wearing['ODAXIS'];
            $result["OSSPH_$i"] = $wearing['OSSPH'];
            $result["OSCYL_$i"] = $wearing['OSCYL'];
            $result["OSAXIS_$i"] = $wearing['OSAXIS'];
            $result["ODMIDADD_$i"] = $wearing['ODMIDADD'];
            $result["OSMIDADD_$i"] = $wearing['OSMIDADD'];
            $result["ODADD_$i"] = $wearing['ODADD'];
            $result["OSADD_$i"] = $wearing['OSADD'];
            $result["ODVA_$i"] = $wearing['ODVA'];
            $result["OSVA_$i"] = $wearing['OSVA'];
            $result["ODNEARVA_$i"] = $wearing['ODNEARVA'];
            $result["OSNEARVA_$i"] = $wearing['OSNEARVA'];
            $result["ODPRISM_$i"] = $wearing['ODPRISM'];
            $result["OSPRISM_$i"] = $wearing['OSPRISM'];
            $result["RX_TYPE_$i"] = $wearing['RX_TYPE'];
            $result["W_$i"] = '1';
            }
        }
        $result["json"] = json_encode($result);
...
function display($pid,$encounter,$category_value) {
    ...
    global $documents;
       /**
        *   Each document will need a designator as to the section it belongs in.
    ...
    <nav class="navbar-fixed-top navbar-custom navbar-bright navbar-inner" role="banner" role="navigation" style="margin-bottom: 0;z-index:1999999;font-size: 1.4em;">
        ...
        <div class="container-fluid" style="margin-top:0px;padding:2px;">
            ...
            <div class="navbar-collapse collapse" id="oer-navbar-collapse-1">
                <ul class="navbar-nav">
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            <li id="menu_PRINT_narrative_2" name="menu_PRINT_report_2"><a id="BUTTON_PRINT_report_2" target="_new" href="/openemr/interface/patient_file/report/custom_report.php?printable=1&pdf=1&<?php echo $form_folder."_".$form_id."=".$encounter; ?>"><?php echo xlt("Print/Store PDF"); ?></a></li>
                            <li class="divider"></li>
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li id="menu_Undo" name="menu_Undo"> <a  id="BUTTON_Undo_menu" href="#"> <?php echo xlt("Undo"); ?> <span class="menu_icon"><?php echo xlt('Ctl-Z'); ?></span></a></li>
                            <li id="menu_Redo" name="menu_Redo"> <a  id="BUTTON_Redo_menu" href="#"> <?php echo xlt("Redo"); ?> <span class="menu_icon"><?php echo xlt('Ctl-Shift-Z'); ?></span></a></li>
                        </ul>
                    </li> 
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li id="menu_TEXT" name="menu_TEXT" class="active"><a><?php echo xlt("Text"); ?><span class="menu_icon"><?php echo xlt('Ctl-T'); ?></span></a></li>
                            <li id="menu_DRAW" name="menu_DRAW"><a id="BUTTON_DRAW_menu" name="BUTTON_DRAW_menu"><?php echo xlt("Draw"); ?><span class="menu_icon"><?php echo xlt('Ctl-D'); ?></span></a></li>
                            <li id="menu_QP" name="menu_QP"><a id="BUTTON_QP_menu" name="BUTTON_QP_menu"><?php echo xlt("Quick Picks"); ?><span class="menu_icon"><?php echo xlt('Ctl-B'); ?></span></a></li>
                            <li id="menu_PRIORS" name="menu_PRIORS"><a><?php echo xlt("Prior Visits"); ?><span class="menu_icon"><?php echo xlt('Ctl-P'); ?></span></a></li>
                            <li id="menu_KB" name="menu_KB"><a><?php echo xlt("Shorthand"); ?><span class="menu_icon"><?php echo xlt('Ctl-K'); ?></span></a></li>
                            <li class="divider"></li>
                            ...
                            */
                            ...
                            <li id="menu_fullscreen" name="menu_fullscreen" <?php echo $fullscreen; ?>>
                                <a onclick="top.restoreSession();openNewForm('<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/encounter/load_form.php?formname=fee_sheet');dopopup('<?php echo $_SERVER['REQUEST_URI']. '&display=fullscreen&encounter='.$encounter; ?>');" class=""><?php echo xlt('Fullscreen'); ?></a>
                            </li>
                            <?php } ?>
                        </ul>
                    </li> 
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            ...
                            <li role="presentation"><a target="RTop" role="menuitem" tabindex="-1" 
                                ...
                                <li><?php echo  $episode .= '<a href="/openemr/interface/forms/'.$form_folder.'/css/AnythingSlider/simple.php?display=i&encounter='.$encounter.'&category_name=OTHER&panel1-1">
                ...
                <ul >
                    <li style="position:absolute;right:150px;"><span id="active_flag" name="active_flag" style="margin-right:15px;color:red;"> <?php echo xlt('Active Chart'); ?> </span>
...
function menu_overhaul_left($pid,$encounter) {
    ...
    <div class="borderShadow" style="font-size:1.2em;width:70%;display:inline-block;">
        <div id="left_menu" name="left_menu" class="col-md-4" style="font-size:1.0em;">
           ...
           // $PCP = $prov['fname']." ".$prov['lname'];
            ...
            <table style="border:1pt;font-size:1.0em;">
                ...
                <tr><td class="right" nowrap><b><?php echo xlt("Referred By"); ?>:</b>&nbsp;</td><td style="font-size:0.8em;">&nbsp;
                    <?php $query="SELECT  *,field_id as ref_providerID FROM layout_options WHERE form_id = 'DEM' AND uor > 0 AND field_id = 'providerID' ORDER BY seq";
                        $group_fields_query = sqlStatement($query);
                        while ($group_fields = sqlFetchArray($group_fields_query)) {
                            $group_fields['edit_options']='0'; //disable the select field here
...
function menu_overhaul_bottom($pid,$encounter) {
    ?><div class="navbar-custom" style="width:100%;height:25px;position:relative;border-top:1pt solid black;bottom:0px;">&nbsp;</div><?php
}
...
function report_header($pid,$direction='shell') {
    ...
    *******************************************************************/
    ...
    ?>
    <span style="font-weight:bold;font-size:1.4em;"><?php echo text($facility['name']); ?></span><br />
    ...
    <?php 
     ...
     <span class='title' style="position:absolute;top:25px;right:25px;">
        <a href="javascript:window.close();"><?php echo text($titleres['fname']) . " " . text($titleres['lname']); ?></a><br />
 ...
 *  The clinical fields are found in table list_options with list_id = Coding_Fields_Eye_Form_
 *  The clinical terms to mine for are in table list_options with list_id = Coding_Terms_Eye_Form
...
function start_your_engines($FIELDS) {//pass an assoc array of fields with terms in it and search them
    ...
    global $PMSFH;
    if (!$PMFSH) $PMSFH = build_PMSFH($pid);
    $query = "select * from list_options where list_id ='Coding_Fields_Eye_Form' Order by seq";
    ...
    $query = "SELECT * FROM list_options WHERE list_id = 'Coding_Terms_Eye_Form' order by seq";
    $result = sqlStatement($query);
    ...
    foreach ($clinical_terms as $amihere) {
        ...
        if (stripos($FIELDS[$amihere['location']],$term) !==false) { 
            ...
            if (isset($positives[$amihere['location']]) > '') { //true if anything was already found in this field
                //do any of the previous hits contain this term already?
            ...
            if (($amihere['codes'] > '') && ($option_values=="")) { //did the user define a code for this term in list Coding_Terms_Eye_Form?  
                    ...
                    foreach ($options as $option) {
                        ...
                        if ($option=="DM") {
                            ...
                            foreach( $PMSFH[0]['PMH'] as $k=>$v ){ 
                                if ( stripos($v['codedesc'],xlt("diabetes"))) {
                                    $DM_code = $v['codedesc'];
                                    $within_array = 'yes';
                                }
                            }
                            if ($within_array =="yes") {
                                if (stripos($DM_code,"1")) {
                                    $DM_text = xlt("Type 1 diabetes mellitus{{exact phrase found in ICD code description}}");
                                    $label = xlt("DM 1{{abbreviation for diabetes mellitus type 1}}");
                                } else if (stripos($DM_code,"2")) {
                                    $DM_text = xlt("Type 2 diabetes mellitus{{exact phrase found in ICD code description}}");
                                    $label = xlt("DM 2{{abbreviation for diabetes mellitus type 2}}");
                                } else {
                                    $DM_text = xlt("Other specified diabetes{{exact phrase found in ICD code description}}");
                                     $label = xlt("DM{{abbreviation for diabetes mellitus, other or unspecified}}");
                               }
                            ...
                            if ((stripos($FIELDS[$location],"flat") ===false) && (stripos($FIELDS[$location],"CSME") !==false)) {
                                $MAC_text = xlt("with macular edema{{exact phrase found in ICD code description}}");
                                $hit_CSME=xlt("w/{{abbreviation for the word with}}")." ".xlt("CSME{{abbreviation for clinically significant macular edema}}");
                            } else {
                                $MAC_text=xlt("without macular edema{{exact phrase found in ICD code description}}");
                                $hit_CSME=xlt("w/o{{abbreviation for the word without}}")." ".xlt("CSME{{abbreviation for clinically significant macular edema}}");
                            }
                            //is there (NVD or NVE) or BDR?
                            $NVD    = xlt("NVD{{abbreviation for neovascularization of the disc}}");
                            $NVE    = xlt("NVE{{abbreviation for neovascularization elsewhere}}");
                            $PPDR   = xlt("PPDR{{abbreviation for preproliferative diabetic retinopathy}}");
                            $PDR    = xlt("PDR{{abbreviation for proliferative diabetic retinopathy}}");
                            $BDR    = xlt("BDR{{abbreviation for background diabetic retinopathy}}");
                            $IRMA   = xlt("IrMA{{abbreviation for Intraretinal microvascular abnormalities}}");
                            if ((stripos($FIELDS[$location1],$NVD) !==false) ||
                                (stripos($FIELDS[$location2],$NVE) !==false) ||
                                (stripos($FIELDS[$location3],$NVE) !==false)) {
                                $DX=xlt("with proliferative{{exact phrase found in ICD code description}}");
                                $label = $label." ".xlt("w/{{abbreviation for the word with}}")." ".xlt("PDR{{abbreviation for proliferative diabetic retinopathy}}")." ".$hit_CSME;
                                $hit_PDR[$side]='1';
                            } else if (
                            ...
                            {
                                $DX=xlt("with severe nonproliferative{{exact phrase found in ICD code description}}");
                                $label = $label." ".xlt("w/{{abbreviation for the word with}}")." ".xlt("PPDR{{abbreviation for preproliferative diabetic retinopathy}}")." ".$hit_CSME;
                                $hit_PPDR[$side]='1';
                            } else if (
                                (stripos($FIELDS[$location],$BDR) !==false) ||
                                (stripos($FIELDS[$location2],$BDR) !==false)) 
                                {
                                    $trace = xlt("tr{{abbreviation of the word trace}}");
                                if  (
                                    (stripos($FIELDS[$location],$trace." ".$BDR) !==false) ||
                                    (stripos($FIELDS[$location2],"+1 ".$BDR) !==false) || 
                                    (stripos($FIELDS[$location],$trace." ".$BDR) !==false) ||
                                    (stripos($FIELDS[$location2],"+1 ".$BDR) !==false)) {   
                                    $DX=xlt("with mild nonproliferative{{exact phrase found in ICD code description}}");
                                    $label = $label." ".xlt("w/{{abbreviation for the word 'with'}}")." ".xlt("mild")." ".xlt("BDR{{abbreviation for background diabetic retinopathy}}")." ".$hit_CSME;
                                    $hit_BDR[$side]='1';
                                } else {
                                    $DX=xlt("with moderate nonproliferative{{exact phrase found in ICD code description}}");
                                    $label = $label." ".xlt("w/{{abbreviation for the word 'with'}}")." ".xlt("mod{{abbreviation for the word 'moderate'}}")." ".xlt("BDR{{abbreviation for background diabetic retinopathy}}")." ".$hit_CSME;
                                    $hit_BDR[$side] = '1';
                                }
                            } 
                            ...
                            if (stripos($FIELDS[$location],"CRVO") !==false) {
                               // this is a CRVO, look up code for location
                                $terms = xlt("CRVO{{central retinal vein occlusion}}");
                               ...
                               // this is a BRVO, look up code for location
                                ...
                                $terms = xlt("BRVO{{branch retinal vein occlusion}}")." ".$term;
                                if (isset($code_found)) { //there are matches, present them to the Builder
                                    foreach ($code_found as $found) {
                                        list($sub_term,$newdata) = coding_engine($terms,$found,$location,$side1);
                                        $codes_found[$sub_term][] = $newdata;
                            ...
                            if (($term=="CSME") && ($hit_RVO[$location]=='1')) {
                                ...
                                $terms = xlt("Vein occlusion and macular edema");
                                if (isset($code_found)) { //there are matches, present them to the Builder
                                    foreach ($code_found as $found) {
    ...
    // It also gets "horsepower" from the POH and PMH findings.
...
function coding_engine($term, $code_found,$location,$side='') {
    ...
    //(preg_match("/right/",$code_desc))? $side = xlt('OD') : $side = xlt('OS');
...
/**
 *  This function is run if the categories are not set up corretly yet.
 *  In lieu of performing these tasks for everyone in database.sql, we do it here.
 *  Thus only if the end user actively uses this form will the tables and categories be created. 
 */
function first_run() {
    // Check to see what is present.  On first run create what is needed.
    // Anything the database.sql file adds needs to be checked in case they are adding this form to an old install.

    // Check to see if the Categories are setup. They are not needed hence not setup unless this form is used.
    // We'd like to create two subcategories under 'Medical Record': Imaging and Encounters if they are not present.  
    // If Imaging is not here, then this function is run.
    // If Imaging_ID == 1, we have been here, done that, so move along.
    ...
    if ($Imaging_ID < '1') {
        ...
        if ($ID['count'] != '1') {
            ...
            $queries = "INSERT INTO list_options (`list_id`,`option_id`,`title`) VALUES ('lists','issue_subtypes','Issue Subtypes');
                    INSERT INTO list_options (list_id, option_id, notes,title, seq) VALUES ('issue_subtypes', 'default', '', '', 1);
                    INSERT INTO list_options (list_id, option_id, notes,title, seq) VALUES ('issue_subtypes', 'eye', 'eye', 'Eye',2);";
            mysql_multiquery($queries);
        } 

        //check to see if subtype column is added to lists, if not add it.
        $query = "SELECT COUNT(*) as count
                        FROM INFORMATION_SCHEMA.COLUMNS
                        WHERE   TABLE_NAME = 'lists' AND 
                                COLUMN_NAME = 'subtype'";
        $result = sqlStatement($query);
        $ID = sqlFetchArray($result);
        if ($ID['count'] != '1') {
            $query ="ALTER TABLE `lists`  ADD `subtype` VARCHAR(31) DEFAULT ''";
            $result = sqlStatement($query);    
        }

        //check to see if activity column is added to lists, if not add it.
        $query = "SELECT COUNT(*) as count
                        FROM INFORMATION_SCHEMA.COLUMNS
                        WHERE   TABLE_NAME = 'lists' AND 
        ...
        if ($ID['count'] != '1') {
            ...
            $query ="ALTER TABLE `list_options` ADD COLUMN `activity` TINYINT DEFAULT 1 NOT NULL";
            #EndIf
            $result = sqlStatement($query);    
        }
        $query = "select seq from list_options where list_id like 'CTLManufacturer'";
        $result = sqlStatement($query);
        $ID = sqlFetchArray($result);
        if ($ID['seq'] < '1') {
            $query = "SELECT max(seq) as maxseq FROM list_options WHERE list_id= 'lists'";
            ...
            $seq3 = $seq +3;
            $seq4 = $seq +4;
            $query = "INSERT IGNORE INTO `openemr`.`list_options` 
            ...
            sqlStatement($query);
            $query = "INSERT IGNORE INTO list_options
            ...
            sqlStatement($query);
            //these are the fields for generating codes from the clinical fields for the coding engine
            $query = "INSERT IGNORE INTO list_options ( list_id, option_id, title, seq, is_default, option_value, mapping, notes, codes, subtype) VALUES
                    ('lists', 'Coding_Fields_Eye_Form', 'Coding Fields Eye Form', ".$seq3.", 1, 0, '', '', '', ''),
                    ('lists', 'Coding_Terms_Eye_Form', 'Coding Terms Eye Form',".$seq4.", 1, 0, '', '', '', ''),
                    ('Coding_Fields_Eye_Form', 'RUL', 'RUL', 1, 0, 0, '', 'right upper eyelid', '', ''),
                    ('Coding_Fields_Eye_Form', 'RLL', 'RLL', 2, 0, 0, '', 'right lower eyelid', '', ''),
                    ('Coding_Fields_Eye_Form', 'LUL', 'LUL', 3, 0, 0, '', 'left upper eyelid', '', ''),
                    ('Coding_Fields_Eye_Form', 'LLL', 'LLL', 4, 0, 0, '', 'left lower eyelid', '', ''),
                    ('Coding_Fields_Eye_Form', 'RBROW', 'RBROW', 5, 0, 0, '', 'forehead', '', ''),
                    ('Coding_Fields_Eye_Form', 'LBROW', 'LBROW', 6, 0, 0, '', 'forehead', '', ''),
                    ('Coding_Fields_Eye_Form', 'RMCT', 'RMCT', 7, 0, 0, '', 'canthus', '', ''),
                    ('Coding_Fields_Eye_Form', 'LMCT', 'LMCT', 8, 0, 0, '', 'canthus', '', ''),
                    ('Coding_Fields_Eye_Form', 'RBROW_unspec', 'RBROW', 9, 0, 0, '', 'unspecified', '', ''),
                    ('Coding_Fields_Eye_Form', 'LBROW_unspec', 'LBROW', 10, 0, 0, '', 'unspecified', '', ''),
                    ('Coding_Fields_Eye_Form', 'RADNEXA', 'RADNEXA', 11, 0, 0, '', 'unspecified', '', ''),
                    ('Coding_Fields_Eye_Form', 'LADNEXA', 'LADNEXA', 12, 0, 0, '', 'unspecified', '', ''),
                    ('Coding_Fields_Eye_Form', 'RCONJ', 'RCONJ', 13, 0, 0, '', 'right conjunctiva', '', ''),
                    ('Coding_Fields_Eye_Form', 'LCONJ', 'LCONJ', 14, 0, 0, '', 'left conjunctiva', '', ''),
                    ('Coding_Fields_Eye_Form', 'RCORNEA', 'RCORNEA', 15, 0, 0, '', 'right cornea', '', ''),
                    ('Coding_Fields_Eye_Form', 'LCORNEA', 'LCORNEA', 16, 0, 0, '', 'left cornea', '', ''),
                    ('Coding_Fields_Eye_Form', 'RAC', 'RAC', 17, 0, 0, '', 'right anterior chamber', '', ''),
                    ('Coding_Fields_Eye_Form', 'RLENS', 'RLENS', 18, 0, 0, '', 'right lens', '', ''),
                    ('Coding_Fields_Eye_Form', 'LLENS', 'LLENS', 19, 0, 0, '', 'left lens', '', ''),
                    ('Coding_Fields_Eye_Form', 'RIRIS', 'RIRIS', 30, 0, 0, '', 'right iris', '', ''),
                    ('Coding_Fields_Eye_Form', 'LIRIS', 'LIRIS', 31, 0, 0, '', 'left iris', '', ''),
                    ('Coding_Fields_Eye_Form', 'RDISC', 'RDISC', 32, 0, 0, '', 'right', '', ''),
                    ('Coding_Fields_Eye_Form', 'LDISC', 'LDISC', 33, 0, 0, '', 'left', '', ''),
                    ('Coding_Fields_Eye_Form', 'RMAC', 'RMAC', 34, 0, 0, '', 'right macula', '', ''),
                    ('Coding_Fields_Eye_Form', 'LMAC', 'LMAC', 35, 0, 0, '', 'left macula', '', ''),
                    ('Coding_Fields_Eye_Form', 'RVESSELS', 'RVESSELS', 36, 0, 0, '', 'left', '', ''),
                    ('Coding_Fields_Eye_Form', 'RPERIPH', 'RPERIPH', 37, 0, 0, '', 'right retina', '', ''),
                    ('Coding_Fields_Eye_Form', 'LPERIPH', 'LPERIPH', 38, 0, 0, '', 'left retina', '', ''),
                    ('Coding_Terms_Eye_Form', 'dermato_RUL', 'dermatochalasis', 1, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'dermato_RLL', 'dermatochalasis', 2, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'dermato_LUL', 'dermatochalasis', 3, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'dermato_LLL', 'dermatochalasis', 4, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'ptosis_RUL', 'ptosis', 5, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'ptosis_LUL', 'ptosis', 6, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'chalazion_RUL', 'chalazion', 7, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'chalazion_RLL', 'chalazion', 8, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'chalazion_LUL', 'chalazion', 9, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'chalazion_LLL', 'chalazion', 10, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'cicectr_RUL', 'cicatricial ectropion', 11, 0, 0, '', 'RUL', 'ICD10:H02.111', ''),
                    ('Coding_Terms_Eye_Form', 'cicectr_RLL', 'cicatricial ectropion', 12, 0, 0, '', 'RLL', 'ICD10:H02.112', ''),
                    ('Coding_Terms_Eye_Form', 'cicectr_LUL', 'cicatricial ectropion', 13, 0, 0, '', 'LUL', 'ICD10:H02.114', ''),
                    ('Coding_Terms_Eye_Form', 'cicectr_LLL', 'cicatricial ectropion', 14, 0, 0, '', 'LLL', 'ICD10:H02.115', ''),
                    ('Coding_Terms_Eye_Form', 'spasectr_RUL', 'spastic ectropion', 15, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasectr_RLL', 'spastic ectropion', 16, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasectr_LUL', 'spastic ectropion', 17, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasectr_LLL', 'spastic ectropion', 18, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'cicentr_RUL', 'cicatricial entropion', 19, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'cicentr_RLL', 'cicatricial entropion', 20, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'cicentr_LUL', 'cicatricial entropion', 21, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'cicentr_LLL', 'cicatricial entropion', 22, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasentr_RUL', 'spastic entropion', 15, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasentr_RLL', 'spastic entropion', 16, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasentr_LUL', 'spastic entropion', 17, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'spasentr_LLL', 'spastic entropion', 18, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'ect_RUL', 'ectropion', 25, 0, 0, '', 'RUL', 'ICD10:H02.101', ''),
                    ('Coding_Terms_Eye_Form', 'ect_RLL', 'ectropion', 26, 0, 0, '', 'RLL', 'ICD10:H02.102', ''),
                    ('Coding_Terms_Eye_Form', 'ect_LUL', 'ectropion', 27, 0, 0, '', 'LUL', 'ICD10:H02.104', ''),
                    ('Coding_Terms_Eye_Form', 'ect_LLL', 'ectropion', 28, 0, 0, '', 'LLL', 'ICD10:H02.105', ''),
                    ('Coding_Terms_Eye_Form', 'ent_RUL', 'entropion', 29, 0, 0, '', 'RUL', 'ICD10:H02.001', ''),
                    ('Coding_Terms_Eye_Form', 'ent_RLL', 'entropion', 30, 0, 0, '', 'RLL', 'ICD10:H02.002', ''),
                    ('Coding_Terms_Eye_Form', 'ent_LUL', 'entropion', 31, 0, 0, '', 'LUL', 'ICD10:H02.004', ''),
                    ('Coding_Terms_Eye_Form', 'ent_LLL', 'entropion', 32, 0, 0, '', 'LLL', 'ICD10:H02.005', ''),
                    ('Coding_Terms_Eye_Form', 'trich_RUL', 'trichiasis', 33, 0, 0, '', 'RUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'trich_RLL', 'trichiasis', 34, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'trich_LUL', 'trichiasis', 35, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'trich_LLL', 'trichiasis', 36, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'stye_RUL', 'stye', 37, 0, 0, '', 'RUL', 'ICD10:H00.011', ''),
                    ('Coding_Terms_Eye_Form', 'stye_RLL', 'stye', 38, 0, 0, '', 'RLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'stye_LUL', 'stye', 39, 0, 0, '', 'LUL', '', ''),
                    ('Coding_Terms_Eye_Form', 'stye_LLL', 'stye', 40, 0, 0, '', 'LLL', '', ''),
                    ('Coding_Terms_Eye_Form', 'papillae_ODCONJ', 'papillae', 41, 0, 0, '','ODCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'papillae_OSCONJ', 'papillae', 42, 0, 0, '','OSCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'folllicles_ODCONJ', 'folllicles', 43, 0, 0, '','ODCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'folllicles_OSCONJ', 'folllicles', 44, 0, 0, '','OSCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'pterygium_ODCONJ', 'pterygium', 45, 0, 0, '','ODCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'pterygium_OSCONJ', 'pterygium', 46, 0, 0, '','OSCONJ', '', ''),
                    ('Coding_Terms_Eye_Form', 'abrasion_ODCORNEA', 'abrasion', 47, 0, 0, '','ODCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'abrasion_OSCORNEA', 'abrasion', 48, 0, 0, '','OSCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'FB_ODCORNEA', 'FB', 49, 0, 0, '','ODCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'FB_OSCORNEA', 'FB', 50, 0, 0, '','OSCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'dendrite_ODCORNEA', 'dendrite', 51, 0, 0, '','ODCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'dendrite_OSCORNEA', 'dendrite', 52, 0, 0, '','OSCORNEA', '', ''),
                    ('Coding_Terms_Eye_Form', 'MDF_ODCORNEA', 'MDF', 53, 0, 0, '','ODCORNEA','',''),
                    ('Coding_Terms_Eye_Form', 'MDF OSCORNEA', 'MDF', 54, 0, 0, '','OSCORNEA','',''),
                    ('Coding_Terms_Eye_Form', 'NS_ODLENS', 'NS', 55, 0, 0, '','ODLENS', '', ''),
                    ('Coding_Terms_Eye_Form', 'NS_OSLENS', 'NS', 56, 0, 0, '','OSLENS', '', ''),
                    ('Coding_Terms_Eye_Form', 'PSC_ODLENS', 'PSC', 57, 0, 0, '','ODLENS', '', ''),
                    ('Coding_Terms_Eye_Form', 'PSC_OSLENS', 'PSC', 58, 0, 0, '','OSLENS', '', ''),
                    ('Coding_Terms_Eye_Form', 'PCIOL_ODLENS', 'PCIOL', 59, 0, 0, '','ODLENS','',''),
                    ('Coding_Terms_Eye_Form', 'PCIOL OSLENS', 'PCIOL', 60, 0, 0, '','OSLENS','',''),
                    ('Coding_Terms_Eye_Form', 'hyphema ODAC', 'hyphema', 61, 0, 0, '','ODAC', '', ''),
                    ('Coding_Terms_Eye_Form', 'hyphema OSAC', 'hyphema', 62, 0, 0, '','OSAC', '', ''),
                    ('Coding_Terms_Eye_Form', 'tear_ODPERIPH', 'tear', 63, 0, 0, '','ODPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'tear_OSPERIPH', 'tear', 64, 0, 0, '','OSPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'horseshoe_ODPERIPH', 'horseshoe', 65, 0, 0, '','ODPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'horseshoe OSPERIPH', 'horseshoe', 66, 0, 0, '','OSPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'NVD_ODDISC', 'NVD:DM', 67, 0, 0, '','ODDISC', '', ''),
                    ('Coding_Terms_Eye_Form', 'NVD_OSDISC', 'NVD:diabetes-CSME', 68, 0, 0, '','OSDISC', '', ''),
                    ('Coding_Terms_Eye_Form', 'NVE_ODPERIPH', 'NVE:DM', 69, 0, 0, '','ODPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'NVE_OSPERIPH', 'NVE:DM', 70, 0, 0, '','OSPERIPH', '', ''),
                    ('Coding_Terms_Eye_Form', 'CSME_ODMACULA', 'CSME:DM|IOL|RVO', 71, 0, 0, '','ODMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'CSME_OSMACULA', 'CSME:DM|IOL|RVO', 72, 0, 0, '','OSMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'PED_ODMACULA', 'PED', 73, 0, 0, '','ODMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'PED_OSMACULA', 'PED', 74, 0, 0, '','OSMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'drusen_ODMACULA', 'drusen', 75, 0, 0, '','ODMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'drusen_OSMACULA', 'drusen', 76, 0, 0, '','OSMACULA', '', ''),
                    ('Coding_Terms_Eye_Form', 'drusen_ODDISC', 'drusen', 77, 0, 0, '','ODDISC', '', ''),
                    ('Coding_Terms_Eye_Form', 'drusen  OSDISC', 'drusen', 78, 0, 0, '','ODDISC', '', '');";
            sqlStatement($query);
        }
    }
