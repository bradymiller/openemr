...
function make_task($ajax_req) {
	...
	if ($task['ID'] && $task['COMPLETED'] =='2') {
		$send['comments'] = xlt('This fax has already been sent twice.  Consider printing the Fax Report and send it manually.');
		...
		if ($task['DOC_TYPE'] == 'Fax') {
			...
			$send['comments'] = xlt('OK, we will resend it.  We just do not know how to do this yet automatically.');
			echo json_encode($send);
			update_taskman($task,'refaxed', '2');
			exit;
		} else {
			$send['comments'] =  xlt("The ".$task['DOC_TYPE']." was stored as a Document.");
			echo json_encode($send);
			exit;
		}
	} else if (!$task['ID']) {
...
function process_tasks($task) {
	...
	/**	
 	 ...
 	 */
	$task = make_document($task); //for now always returns false.
	//echo "Document is made.\n";
	...
	if ($task['DOC_TYPE'] == "Fax") {
		//now return any objects you need to:
		$send['DOC_link'] = "<a href='".$webroot."/openemr/controller.php?document&view&patient_id=".$task['PATIENT_ID']."&doc_id=".$task['DOC_ID']."'
 ...
 *		Some suggest the fax server to fax machine portion of efaxing is not HIPPA, no matter howit is done.
 *		Thus faxing is not HIPPA compliany, and if that affects you, don't deliver this way.
 */
...
function make_document($task) {
	...
	if ($task['DOC_TYPE'] =='Fax') { 
		$category_name = "Communication"; //Faxes are stored in the Docuemnts->Communication category
		$query = "select id from categories where name =?";
    	$ID = sqlQuery($query,array($category_name));
    	$category_id = $ID['id'];
    	
    	$filename = "Fax_".$encounter."_".$task['TO_ID'].".pdf"; 
		while (file_exists($filepath.'/'.$filename)) {
    		$count++;
    		$filename = "FAX_".$encounter."_".$task['TO_ID']."_".$count.".pdf"; 
    	} 
	} else {
		...
		$filename = "Report_".$encounter."_".$task['TO_ID'].".pdf"; 
