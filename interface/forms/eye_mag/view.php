...
$result = sqlStatement($query,array($_SESSION['authId'])); //maybe this should be the providerID?
// preferences are saved according to authID so keep authID on a user basis (tech or doc), not just a doc preferences
while ($prefs= sqlFetchArray($result))   {    
    @extract($prefs);    // overwriting PREFS is OK. Do we need to text($VARIABLE) them? Do it here:
    $$LOCATION = text($VALUE); 
}
...
/*
  ...
  This is done with e-signing - you can't get here when it is e-signed, on to the report.php.
  
  **THIS HAS BEEN IMPLEMENTED** 
  Need to think about this some.  Two windows anywhere with the same chart is not compatible with the autosave feature.
  Data integrity problems will arise.  Maybe the session variable is a good way in case the same user has it opened on a new computer?
  I assume a new window on a second computer will have a new session.  What if the user has two windows opened on the same machine?
  Maybe we need a random number generated each time the form is opened, which is removed from the "locked" field on closure.
  Any new windows meets with a pop-up and failure to save unless overridden.  If overridden, the other window will then
  meet with a popup saying hey buddy, you lost ownership so refresh - none of the last changes are saved.  
  ...
  if ($LOCKED) { we pop-up something to say the form is being edited elsewhere, do you want to take ownership?
  If you don't your changes will not be saved.  Need to allow view without incessant pop-ups.
  Maybe just once? }
 
  I can imagine a doctor has the window opened and would like to see the chart before entering the room.
  Observe as they are entered by the tech?  Maybe there is a visible mode that does this without submitting, and auto refreshes every minute or so?
...
if ($refresh and $refresh != 'fullscreen') {
  if ($refresh == "PMSFH")  { 
    ...
    $send['PMH_panel'] = display_PMSFH('2');//display_PRIOR_section($refresh,$id,$id,$pid);
...
<html>
  <head>
    <title> <?php echo xlt('Chart'); ?>: <?php echo $pat_data['fname']." ".$pat_data['lname']." ".$visit_date; ?></title>
    <!-- jQuery library -->
    ...
    function doscript(type,id,encounter) {
      dlgopen('../../forms/eye_mag/SpectacleRx.php?REFTYPE=' + type + '&id='+id+'&encounter='+ encounter, '_new', 560, 590);
    }
     
    function dispensed(pid) {
      dlgopen('../../forms/eye_mag/SpectacleRx.php?dispensed=1&pid='+pid, '_new', 560, 590);
    }
    // This invokes the find-code popup.
    function sel_diagnosis(target) {
      IMP_target = target;
      <?php
      ...
      <?php 
        /*  
         *  This builds the IMPPLAN_items variable for the Impression/Plan zone.
         *  This is generated to pass to javascript should any IMP/PLAN items already exits for this form_id
         *  If this is the first time the form is opened, it will be blank and do nothing
         */
          ...
          if ($IMPPLAN_items) { ?>
    var IMPPLAN_items = <?php echo json_encode($IMPPLAN_items); ?>; 
          <?php } ?>
  </script>
      ...
      <meta name="viewport" content="width=device-width, initial-scale=1">
      <?php
        //this places the openEMR menu/buttons at the top
        //we need this for some code regarding the DOM, not sure what but we don't need it visible...
        //There is a bug in this new_form.php too that needs tracking down.
        //This error appears after following a link from this page.  Even the openEMR menu which shows 
        //if selecting another type of form via the menu (EyeExam Menu).  
        //None of the links work on subsequent pages. 
        /*
          09:06:24.891 TypeError: parent.Forms is undefined
          openNewForm() load_form.php:16
          onclick() load_form.php:1
          1 load_form.php:16:2
...
?><br /><br />
    <!-- if this is in a frame, this button allow us to go fullscreen. Hide this if fullscreen though. -->  
    <?php 
    if ($display != "fullscreen") {   
      ?> 
          <i onclick="top.restoreSession();openNewForm('<?php echo $GLOBALS['webroot']; ?>/interface/patient_file/encounter/load_form.php?formname=fee_sheet');dopopup('<?php echo $_SERVER['REQUEST_URI']. '&display=fullscreen&encounter='.$encounter; ?>');" 
            class="fa fa-lg fa-plus-square-o" style="position:fixed;top:5px;right: 20px;z-index:10000001;"></i>
      <?php 
     }  
    ?>
      
    <div id="page-wrapper" style="margin: 0px 0px 0px 0px;text-align:center;">
      ...
      <div id="Layer3" name="Layer3" class="container-fluid" style="text-align:center;padding:0px 30px;">
        ...
        if ($output != '') {
          //get any orders from the last visit
        ...
        if ($display =="fullscreen") {  
          ...
          if ($PLAN_today or $reason['pc_hometext']) {
            //format PLAN to allow checkboxes for techs to show done?
            //any other way to automate it that these tasks are done to save the techs from these click actions
            ?>
              <div class="borderShadow" style="margin:0 auto;padding:5px;width:33%;text-align:center;min-width:400px;">
                <table style="text-align:left;margin:auto;">
                <tr>
                  <td class="right" style="vertical-align:top;" nowrap><b><?php echo xlt("Appointment Reason"); ?>:</b>&nbsp;</td>
        ...
        <form method="post" action="<?php echo $rootdir;?>/forms/<?php echo $form_folder; ?>/save.php?mode=update" id="eye_mag" class="eye_mag pure-form" name="eye_mag">
          <div id="Layer1" name="Layer1" class="display">
            ...
            <div class="body_top text-center row" id="form_container" name="form_container">
              ...
              <!-- start first div -->
              <div id="first" name="first" class="text_clinical" style="display:inline-block;">
                <!-- start    HPI spinner -->
                ...
                <!-- start    HPI_PMH row -->
                <div id="HPIPMH_sections" name="HPIPMH_sections" style="display:inline-block;margin: 10 auto;width:100%;text-align:center;" class="nodisplay" >   
                  <!-- start    HPI_section -->
                        ...
                        <b><?php echo xlt('HPI'); ?>:</b> <i class="fa fa-help"></i><br />
                          <div id="tabs_wrapper" >
                            <div id="tabs_container" style="z-index:3;margin-top:5px;">
                              <ul id="tabs">
                                <li id="tab1_CC" style="z-index:2;" class="active" ><a type="button" class="fa fa-check" href="#tab1"> <?php echo xlt('CC{{Chief Complaint}}'); ?> 1</a></li>
                            ...
                            <div id="tabs_content_container" style="z-index:1;height:303px;" class="borderShadow">
                              <div id="tab1_CC_text" class="tab_content">
                                <table border="0" width="100%" cellspacing="0" cellpadding="0" style="min-height: 232px;text-align:left;">
                                  <tr>
                                  ...
                                  <tr>
                                    ...
                                    echo "PMH items flagged as Chronic with a comment regarding status will automatically appear here.";?>" style="height:1in;font-weight:600;vertical-align:text-top;"><?php echo xlt('Chronic Problems') ?>:</span>
                                      <span class="kb_off"><br /></span></span><div class="kb kb_right">CHRONIC1</div>
                                  ...
                                  <table border="0" width="100%" cellspacing="0" cellpadding="0" 
                                    ...
                                    <tr>
                                      <td class="" style="vertical-align:top;padding:10px;" colspan="2">
                                        <b><span title="<?php echo xla('In the patient\'s words'); ?>"><?php echo xlt('Chief Complaint');  echo $IMPPLAN_items; ?> 2:
                                        </span>  </b>
                                        <br />
                  ...
                  <div id="PMH_1" name="PMH_1" class="<?php echo attr($display_Add); ?> clear_both">
                    ...
                    <div id="PMH_left" name="PMH_left" class="exam_section_left borderShadow">
                      <div id="PMH_left_text" style="height: 2.5in;text-align:left;" class="TEXT_class">
                        <b class="left"><?php echo xlt('PMSFH'); ?>:</b> <i class="fa fa-help"></i><br />
                        ...
                        <i class="closeButton_4 fa fa-user-md fa-sm fa-2" name="Shorthand_kb" title="<?php echo xla("Open the Shorthand Window and display Shorthand Codes"); ?>"></i>
                        <a class="closeButton fa fa-list" id="right-panel-link" name="right-panel-link" href="#right-panel"></a>
                          ...
                          <div id="Enter_PMH" name="Enter_PMH" class="PMH_class" style="text-align:left;vertical-align:middle; min-height: 2.3in;font-size:0.7em;">
                            <center>
                              <iframe id="iframe" name="iframe" 
                                src="/openemr/interface/forms/eye_mag/a_issue.php?uniqueID=<?php echo $uniqueID; ?>&form_type=POH&pid=<?php echo $pid; ?>&encounter=<?php echo $encounter; ?>&form_id=<?php echo $form_id; ?>" 
                                title="PMSFH Form" width="435" height="320" scrolling = "yes" frameBorder = "0" >
                              </iframe>
                            </center>
                          </div>
                        </div>
                    ...
                    <div id="PMH_right" name="PMH_right" class="exam_section_right borderShadow">
                      ...
                      <!--<div class="QP_block borderShadow " style="text-align:center;min-height: 300px;padding:4 10 4 10;font-weight:600;">
                                <span onclick="$('.kb').toggleClass('nodisplay');" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke">Shorthand</span>&nbsp;<a onclick="goto_url('<?php echo $GLOBALS['webroot']; ?>/interface/forms/eye_mag/help.php?zone=ext');">
              ...
              <div class="clear_both row" style="display: inline-block;margin:10px auto;" id="LayerTechnical_sections_1" name="LayerTechnical_sections" >
                ...
                <div id="LayerMood" class="vitals" style="width: 136px; min-height: 1.05in;border: 1.00pt solid #000000;text-align:left;padding-left:10px;float:left;">
                  ...
                  <label for="alert" class="input-helper input-helper--checkbox">Alert</label><br />
                  <input type="checkbox" name="oriented" id="oriented" <?php if ($oriented) echo "checked='checked'"; ?> value="1">
                  <label for="oriented" class="input-helper input-helper--checkbox">Oriented TPP</label><br />
                  <input type="checkbox" name="confused" id="confused" <?php if ($confused) echo "checked='checked'"; ?> value="1">
                  <label for="confused" class="input-helper input-helper--checkbox">Mood/Affect Nml</label><br />
                   
                </div>
                <!-- end of the Mood BOX -->
                
                <!-- start of the VISION BOX -->                  
                <div id="LayerVision" class="vitals" style="width: 2.0in; min-height: 1.05in;padding: 0.02in; border: 1.00pt solid #000000;float:left;">
                  ...
                  <div class="top_right">
                          <span id="tabs" style="font-size:0.5em;">  
                              <ul>
                                  <li id="LayerVision_W_lightswitch" class="<?php echo attr($button_W); ?>" value="Current"><?php echo xlt('W{{Current Rx - wearing}}'); ?></li> | 
                                  <li id="LayerVision_MR_lightswitch" class="<?php echo attr($button_MR); ?>" value="Auto"><?php echo xlt('MR{{Manifest Refraction}}'); ?></li> | 
                                  <li id="LayerVision_CR_lightswitch" class="<?php echo attr($button_AR); ?>" value="Cyclo"><?php echo xlt('AR{{AutoRefraction}}'); ?></li> | 
                                  <li id="LayerVision_CTL_lightswitch" class="<?php echo attr($button_CTL); ?>" value="Contact Lens"><?php echo xlt('CTL{{Contact Lens}}'); ?></li> | 
                                  <li id="LayerVision_ADDITIONAL_lightswitch" class="<?php echo attr($button_ADDITIONAL); ?>" value="Additional"><?php echo xlt('Add.{{Additional Measurements}}'); ?></li> | 
                                  <li id="LayerVision_VAX_lightswitch" class="<?php echo attr($button_VAX); ?>" value="Visual Acuities"><?php echo xlt('Va{{Summary of Visual Acuities}}'); ?></li>                           
                              </ul>
                          </span>
                  </div>    
                  ...
                  <div id="Visions_A" name="Visions_A" class="" style="position: absolute; top: 0.35in; text-align:right;right:0.1in; height: 0.72in;  padding: 0in;" >
                      ...
                      <input type="TEXT" style="left: 0.5in; width: 0.35in; height: 0.19in; " tabindex="42" id="WODVA_copy" name="WODVA_copy" value="<?php echo attr($WODVA); ?>">
                      ...
                      <input type="TEXT" style="left: 0.5in; width: 0.35in; height: 0.19in; " tabindex="43" id="WOSVA_copy" name="WOSVA_copy" value="<?php echo attr($WOSVA); ?>">
                ...
                <div id="LayerTension" class="vitals" style="width: 1.5in; height: 1.05in;padding: 0.02in; border: 1.00pt solid #000000;float:left;">
                      
                      <span title="This will display a graph of IOPs over time" id="LayerVision_IOP_lightswitch" name="LayerVision_IOP_lightswitch" class="closeButton fa  fa-line-chart" id="IOP_Graph" name="IOP_Graph"></span>
                      <!-- -->
                      ...
                      <div id="Lyr4.2" style="position: absolute; top: 0.35in; text-align:right;right:0.1in; height: 0.72in;  padding: 0in; border: 1pt black;">
                          ...
                          <input type="text" style="left: 0.5in; width: 0.23in; height: 0.18in; " name="ODIOPFTN" id="ODIOPFTN" value="<?php echo attr($ODIOPTPN); ?>">
                          <br />
                          <b><?php echo xlt('OS'); ?> </b>
              ...
              <div class="sections" name="mid_menu" id="mid_menu" style="font-size:1.0em;white-space:nowrap;margin:10px;">
                ...
                <span id="EXAM_DRAW" name="EXAM_DRAW" value="DRAW" class="borderShadow">
                  ...
                  // $output = priors_select("ALL",$id,$id,$pid); 
                  ($output =='') ? ($title = "There are no prior visits documented to display for this patient.") : ($title="Display old exam findings and copy forward if desired");?>
                ...
                <span id="EXAM_ALL_keyboard_left" name="EXAM_ALL_keyboard_left" class="borderShadow" style="padding-right:5px;width:250px;">
                  <i class="fa fa-user-md fa-sm" name="Shorthand_kb" title="<?php echo xla("Open the Shorthand Window and display fields' Shorthand Codes"); ?>"></i>&nbsp;
                  <span id="BAR_kb" name="BAR_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class=""><b>Shorthand</b>
                  </span>
                  &nbsp;
              ...
              <div style="margin: 0 auto;text-align: center;font-size:1.4em;Xmin-width:300px;" class="kb borderShadow nodisplay" id="EXAM_KB" name="EXAM_KB">   
                ...
                <span class="BAR2_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke"><b>Shorthand</b>
                </span>
              ...
              <div id="REFRACTION_sections" name="REFRACTION_sections" class="row nodisplay clear_both" style="position:relative;display:inline-block;margin:10px auto;max-width:80%;">
                               ...
                               <!-- start IOP chart section -->
                                ...
                                <div id="LayerVision_IOP" class="borderShadow <?php echo $display_IOP; ?>">
                                    <span class="closeButton fa fa-close" id="Close_IOP" name="Close_IOP"></span>
                                     <table id="iopgraph" name "iopgraph" style="min-width:600px;">
                                ...
                                <div id="LayerVision_W" class="refraction borderShadow <?php echo $display_W; ?>">
                                  <i class="closeButton fa fa-close" id="Close_W" name="Close_W" title="<?php echo attr('Close and make this a Preference to stay closed'); ?>"></i>
                                  ...
                                  <i onclick="top.restoreSession();  dispensed('<?php echo attr($pid); ?>');return false;" 
                                     title="<?php echo attr("List of previously dispensed\nSpectacle and Contact Lens Rxs"); ?>" class="closeButton3 fa fa-list-ul"></i>
                                    <table id="wearing" >
                                        <tr>
                                ...
                                <div id="LayerVision_MR" class="refraction borderShadow <?php echo $display_AR; ?>">
                                    <span class="closeButton fa  fa-close" id="Close_MR" name="Close_MR" title="<?php echo attr('Close and make this a Preference to stay closed'); ?>"></span>
                                    ...
                                    <table id="dry_wet_refraction">
                                        ...
                                        <th NOWRAP colspan="2">
                                                <input type="checkbox" name="BALANCED" id="Balanced" value="on" <?php if ($BALANCED =='on') echo "checked='checked'"; ?> tabindex="142">
                                        ...
                                        <tr>
                                            ...
                                            <td><input type=text id="MRODVA"  name="MRODVA" value="<?php echo attr($MRODVA); ?>" tabindex="136"></td>
                                            <td><input type=text id="MRODADD"  name="MRODADD" value="<?php echo attr($MRODADD); ?>" tabindex="138"></td>
                                            <td><input type=text id="MRNEARODVA"  name="MRNEARODVA" value="<?php echo attr($MRNEARODVA); ?>" tabindex="140"> </td>
                                        ...
                                        <tr>
                                            ...
                                            <td><input type=text id="MROSVA"  name="MROSVA" value="<?php echo attr($MROSVA); ?>" tabindex="137"></td>
                                            <td><input type=text id="MROSADD"  name="MROSADD" value="<?php echo attr($MROSADD); ?>" tabindex="139"></td>
                                            <td><input type=text id="MRNEAROSVA"  name="MRNEAROSVA" value="<?php echo attr($MRNEAROSVA); ?>" tabindex="141"></td>
                                    ...
                                    <table >
                                        ...
                                        <tr>
                                            <td><b><?php echo xlt('OD'); ?>:</b></td>
                                            <td><input type=text id="CRODSPH" name="CRODSPH" value="<?php echo attr($CRODSPH); ?>" tabindex="143"></td>
                                            <td><input type=text id="CRODCYL" name="CRODCYL" value="<?php echo attr($CRODCYL); ?>" tabindex="144"></td>
                                            <td><input type=text id="CRODAXIS" name="CRODAXIS" value="<?php echo attr($CRODAXIS); ?>" tabindex="145"></td>
                                            <td><input type=text id="CRODVA" name="CRODVA"  value="<?php echo attr($CRODVA); ?>" tabindex="149"></td>
                                        ...
                                        <tr>
                                            <td><b><?php echo xlt('OS'); ?>:</b></td>
                                            <td><input type=text id="CROSSPH" name="CROSSPH" value="<?php echo attr($CROSSPH); ?>" tabindex="146"></td>
                                            <td><input type=text id="CROSCYL" name="CROSCYL" value="<?php echo attr($CROSCYL); ?>" tabindex="147"></td>
                                            <td><input type=text id="CROSAXIS" name="CROSAXIS" value="<?php echo attr($CROSAXIS); ?>" tabindex="148"></td>
                                            <td><input type=text id="CROSVA" name="CROSVA" value="<?php echo attr($CROSVA); ?>" tabindex="150"></td>
                                            <td colspan="1" style="text-align:left;">
                                ...
                                <div id="LayerVision_CR" class="refraction borderShadow <?php echo $display_Cyclo; ?>">
                                    <span title="<?php echo attr('Close and make this a Preference to stay closed'); ?>" class="closeButton fa  fa-close" id="Close_CR" name="Close_CR"></span>
                                    <i title="<?php echo attr("Dispense Rx"); ?>" class="closeButton2 fa fa-print" onclick="top.restoreSession();doscript('AR',<?php echo attr($pid); ?>,<?php echo attr($encounter); ?>);return false;"></i>
                                ...
                                <div id="LayerVision_CTL" class="refraction borderShadow <?php echo $display_CTL; ?>">
                                    <span title="<?php echo attr("Dispense Rx"); ?>" title="<?php echo attr('Close and make this a Preference to stay closed'); ?>" class="closeButton fa  fa-close" id="Close_CTL" name="Close_CTL"></span>
                                                ...
                                                <div style="box-shadow: 1px 1px 2px #888888;border-radius: 8px; margin: 0 auto 3; position:inline-block; padding-bottom: 3; border: 1.00pt solid #000000; ">
                                                    <table>
                                                        <tr class="" style="vertical-align:bottom;">
                                                            <td></td>
                                                            <td><a href="/openemr/interface/super/edit_list.php?list_id=CTLManufacturer" target="RTop" 
                                                            ...
                                                            <td><a href="/openemr/interface/super/edit_list.php?list_id=CTLSupplier" target="RTop" 
                                                            ...
                                                            <td><a href="/openemr/interface/super/edit_list.php?list_id=CTLBrand" target="RTop" 
                                    ...
                                    <table>
                                        ...
                                        <tr>
                                          ...
                                          <td colspan="6" style="text-align:left;">
                                            <textarea style="width:95%;height:20px;" name="CTL_COMMENTS" id="CTL_COMMENTS" rows="1"><?php echo text($CTL_COMMENTS); ?></textarea>
                                          </td>
                                        </tr>
                                    </table>
                                </div>
                                ...
                                <div id="LayerVision_ADDITIONAL" class="refraction borderShadow <?php echo $display_Add; ?>">
                                    <span title="<?php echo xla("Dispense Rx"); ?>" title="<?php echo xla('Close and make this a Preference to stay closed'); ?>" class="closeButton fa  fa-close" id="Close_ADDITIONAL" name="Close_ADDITIONAL"></span>
                                ...
                                <div id="LayerVision_VAX" class="refraction borderShadow <?php echo $display_Add; ?>">
                                    <span title="<?php echo attr('Close and make this a Preference to stay closed'); ?>" class="closeButton fa  fa-close" id="Close_VAX" name="Close_VAX"></span> 
              ...
              <!-- my reporting div for development only remove for production -->
              ...
              <div style="margin: 0 auto;text-align: center;display:inline-table;max-width: 95%;" class="nodisplay" id="DA_EXAM_sections" name="DA_EXAM_sections">   
                <!-- start External Exam -->
                  ...
                  <div id="EXT_left" class="exam_section_left borderShadow" >
                    <div id="EXT_left_text" style="height: 2.5in;text-align:left;" class="TEXT_class">
                      ...
                      <div class="QP_not nodisplay" id="EXT_keyboard_left" style="position: absolute;bottom:0.05in;right:0.1in;font-size:0.7em;text-align:left;padding-left:25px;">
                        <span id="EXT_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke"><b>Shorthand</b></span>
                  ...
                  <div id="EXT_right" name="EXT_right" class="exam_section_right borderShadow text_clinical">
                      ...
                      <div id="QP_EXT" name="QP_EXT" class="QP_class" style="text-align:left;max-height: 2.5in;">
                          ...
                          <div style="float:left;width:40px;text-align:left;">
                              <span class="eye_button" id="EXT_prefix_1mm" name="EXT_prefix_1mm"  onclick="$('#EXT_prefix').val('1mm').trigger('change');"> 1<?php echo xlt('mm'); ?> </span>  <br />
                              <span class="eye_button" id="EXT_prefix_2mm" name="EXT_prefix_2mm"  onclick="$('#EXT_prefix').val('2mm').trigger('change');"> 2<?php echo xlt('mm'); ?> </span>  <br />
                              <span class="eye_button" id="EXT_prefix_3mm" name="EXT_prefix_3mm"  onclick="$('#EXT_prefix').val('3mm').trigger('change');"> 3<?php echo xlt('mm'); ?> </span>  <br />
                              <span class="eye_button" id="EXT_prefix_4mm" name="EXT_prefix_4mm"  onclick="$('#EXT_prefix').val('4mm').trigger('change');"> 4<?php echo xlt('mm'); ?> </span>  <br />
                              <span class="eye_button" id="EXT_prefix_5mm" name="EXT_prefix_5mm"  onclick="$('#EXT_prefix').val('5mm').trigger('change');"> 5<?php echo xlt('mm'); ?> </span>  <br />
                          ...
                          <div class="QP_block_outer borderShadow nodisplay" style="z-index:1;text-align:center;border:1pt solid black;padding:7 10 7 10;font-weight:600;">
                            <span id="EXT1_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke">Shorthand</span>&nbsp;<a onclick="goto_url('<?php echo $GLOBALS['webroot']; ?>/interface/forms/eye_mag/help.php?zone=ext');">
                            <i title="<?php echo xla('Click for External shorthand Help.'); ?>" class="fa fa-info-circle fa-1"></i></a><br />
                ...
                <div id="ANTSEG_1" class="clear_both">
                  ...
                  <div id="ANTSEG_left" name="ANTSEG_left" class="exam_section_left borderShadow">
                    <div class="TEXT_class" id="ANTSEG_left_text" style="height: 2.5in;text-align:left;">
                              ...
                              <table style="text-align:center;font-size:1.0em;width:170px;padding-left:5px;"> 
                                  ...
                                  <tr>
                                      <td class="right" title="<?php echo xla('Gonio'); ?>">
                                        <div class="kb kb_left">R/LG</div><?php echo xlt('Gonio{{Gonioscopy}}'); ?> </td>
                                  ...
                                  <tr>
                                      <td class="right" title="<?php echo xla('Pachymetry: Central Corneal Thickness'); ?>">
                                        <div class="kb kb_left">R/LPACH</div><?php echo xlt('Pachy{{Pachymetry}}'); ?> </td>
                                  ...
                                  <tr>
                                      <td class="right" title="<?php echo xla('Schirmers I (w/o anesthesia)'); ?>">
                                        <div class="kb kb_left">R/LSCH1</div><?php echo xlt('Schirmers I'); ?> </td>
                                   ...
                                   <tr>
                                      <td class="right" title="<?php echo xla('Schirmers II (w/ anesthesia)'); ?>">
                                        <div class="kb kb_left">R/LSCH2</div><?php echo xlt('Schirmers II'); ?> </td>
                                  ...
                                  <tr style="padding-bottom:15px;">
                                      <td class="right" title="<?php echo xla('Tear Break Up Time'); ?>">
                                        <div class="kb kb_left">R/LTBUT</div><?php echo xlt('TBUT'); ?> </td>
                      ...
                      <div class="QP_not nodisplay" id="ANTSEG_keyboard_left" style="position: absolute;bottom:0.05in;right:0.1in;font-size:0.7em;text-align:left;padding-left:25px;">
                        <span id="ANTSEG_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke"><b>Shorthand</b></span>
                  ...
                  <div id="ANTSEG_right" NAME=="ANTSEG_right" class="exam_section_right borderShadow text_clinical ">
                      ...
                      <div id="QP_ANTSEG" name="QP_ANTSEG" class="QP_class"  style="text-align:left;height: 2.5in;">
                          ...
                          <div class="QP_block borderShadow text_clinical " >
                             <?php
                                ...
                                while ($Select_data= sqlFetchArray($result))   {
                                  $number_rows++;
                                  $string = $Select_data['selection'] ;
                                   $string = (strlen($string) > 11) ? substr($string,0,10).'...' : $string;         
                                   ?> <span>
                          ...
                          <div class="QP_block_outer borderShadow nodisplay" style="z-index:1;text-align:center;border:1pt solid black;padding:7 10 7 10;font-weight:600;">
                            <span id="ANTSEG1_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke">Shorthand</span>
                            &nbsp;
                  ...
                  <div id="RETINA_left" name="RETINA_left" class="exam_section_left borderShadow">
                    <div class="TEXT_class" id="RETINA_left_text" name="RETINA_left_text" style="height: 2.5in;text-align:left;"> 
                      ...
                      <div class="QP_not" id="RETINA_keyboard_left" style="position: absolute;bottom:0.05in;right:0.1in;font-size:0.7em;text-align:left;padding-left:25px;">
                        <span id="RETINA_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke"><b>Shorthand</b></span>
                    ...
                    <div id="QP_RETINA" name="QP_RETINA" class="QP_class" style="text-align:left;height: 2.5in;">
                      ...
                      <div class="QP_block borderShadow text_clinical" >
                          <?php
                              ...
                              $number_rows++; 
                              $string = $Select_data['selection'] ;
                              $string = (strlen($string) > 12) ? substr($string,0,11).'...' : $string;         
                              ?>
                          <span>
                      ...
                      <div class="QP_block_outer borderShadow nodisplay" style="z-index:1;text-align:center;border:1pt solid black;padding:7 10 7 10;font-weight:600;">
                        <span id="RETINA_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke">Shorthand</span>&nbsp;
                        <a onclick="goto_url('<?php echo $GLOBALS['webroot']; ?>/interface/forms/eye_mag/help.php?zone=retina');">
                        ...
                        <div class="TEXT_class" id="NEURO_left_text" name="NEURO_left_text" style="margin:auto 5;min-height: 2.5in;text-align:left;">
                            ...
                            <div style="float:left;margin-top:8px;font-size:0.8em;">
                                ...
                                <div class="borderShadow" style="position:relative;float:right;text-align:center;width:238px;height:250scpx;z-index:1;margin:2 0 2 2;">                               
                                    ...
                                    <div id="NPCNPA" name="NPCNPA">
                                        <table style="position:relative;float:left;text-align:center;margin: 4 2;width:100%;font-size:1.0em;padding:4px;">
                                            <tr style="font-weight:bold;"><td style="width:50%;"></td><td><?php echo xlt('OD'); ?></td><td><?php echo xlt('OS'); ?></td></tr>
                                            <tr>
                                ...
                                <div id="NEURO_MOTILITY" class="text_clinical borderShadow" 
                                    ...
                                    <div style="float:left;left:0.4in;"><?php echo xlt('OD'); ?></div>
                                    <div style="float:right;right:0.4in;"><?php echo xlt('OS'); ?></div><br />
                            ...
                            <div class="QP_not" id="NEURO_keyboard_left" style="position: absolute;bottom:0.05in;right:0.1in;font-size:0.7em;text-align:left;padding-left:25px;">
                              <span id="NEURO_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke"><b>Shorthand</b></span>
                      ...
                      <div id="QP_NEURO" name="QP_NEURO" class="QP_class" style="text-align:left;height: 2.5in;">
                        ...
                        <div style="float:right;top:1.0in;right:0.0in;margin: 0 25;"><br />
                         ...
                         <div style="width:195px;text-align:center;margin:4;font-weight:600;padding:10 2 10 2;">
                          ...
                          <div class="QP_block_outer borderShadow nodisplay" style="z-index:1;text-align:center;border:1pt solid black;padding:7 10 7 10;font-weight:600;">
                              <span id="NEURO1_kb" title="<?php echo xla('Click to display shorthand field names.'); ?>" class="ke">Shorthand</span>&nbsp;<a onclick="goto_url('<?php echo $GLOBALS['webroot']; ?>/interface/forms/eye_mag/help.php?zone=neuro');">
                        ...
                        <div style="position:left;top:0in;left:0.10in;margin: auto;">
                          ...
                          <div class="borderShadow" style="width:195px;text-align:center;border:1pt solid black;padding:4 10 4 10;margin:4;font-weight:600;"><span class="underline"><?php echo xlt('Position of Gaze'); ?></span><br />
                            ...
                            <span class="eye_button_blank" style="font-size:0.8em;"><i class="fa fa-1 fa-reply rotate-left"></i></span> 
                            ...
                            <span class="eye_button_blank" style="font-size:0.8em;"><i class="fa fa-1 fa-share rotate-right"></i></span> 
                ...
                <div id="IMPPLAN_1" class="clear_both"> 
                  ...
                  <div id="IMPPLAN_left" name="IMPPLAN_left" class="exam_section_left borderShadow">
                    <a class="closeButton_4" title="Once completed, view and store this encounter as a PDF file." target="_report" href="/openemr/interface/patient_file/report/custom_report.php?printable=1&pdf=1&<?php echo $form_folder."_".$form_id."=".$encounter; ?>&"><i class="fa fa-file-pdf-o"></i></a>
                    ...
                    <div id="IMPPLAN_left_text" name="IMPPLAN_left_text" style="margin:auto 5;min-height: 2.5in;text-align:left;">
                      <b><?php echo xlt('Impression/Plan'); ?>:</b><div class="kb kb_left">IMP</div> 
                      <div id="IMPPLAN_new" name="IMPPLAN_new" class="HPI_text">
                        <br />
                        <table class="IMPPLAN">
                          <tr>
                            <td class="right bold" style="width:75px;padding-right:10px;vertical-align:top;">New Dx: </td>
                            <td><textarea style="max-width:335px;width:335px;" name="IMP" id="IMP"><?php echo $IMP; ?></textarea></td>
                          </tr>
                        </table>
                      </div>
                 ...
                 <?php
                  ...
                  <div id="IMPPLAN_right" class="exam_section_right borderShadow text_clinical">
                    ...
                    <div id="QP_IMPPLAN" name="QP_IMPPLAN" class="QP_class" style="text-align:left;min-height: 2.5in;padding-right:20px;padding-left:20px;">
                      <span id="iPLAN_BUILD" name="iPLAN_BUILD" style="font-weight:600;">Impression/Plan</span> 
                      <div style="position:absolute;top:0.05in;right:0.241in;">
                      ...
                      <?php
                      /*
                       *  Let's discuss 3. Build automatically through workflows. 
                       *  The Impression/Plan Builder: (build_PMSFH) here.
                       *  Since POH is our area of concern, use $PMSFH[0]['POH'] first then
                       *  $PMSFH[0]['medical_problem'] then
                       *  system to build DXs from documented clinical findings
                       *  to build the IMP/PLAN
                       *    a. Diagnoses are sortable via dragging, to build an order.
                       *    b. Select DX(s), then click the BUILD icon to append the selectd DXs to the bottom of the IMP/Plan list.
                      ...
                      <dt class="borderShadow"><i title="<?php echo xla('Drag the arrow for each diagnosis to sort the list.'); echo "\n"; 
                        echo xla('Select the diagnoses to include in the Impression/Plan.'); echo "\n"; 
                        ...
                        <span id="IMP_start" name="IMP_start">Impression/Plan Builder:</span>
                        <div style="position:absolute;top:3px;right:4px;">
                          <input type="checkbox" id="inc_POH" name="inc_POH" checked><label for='inc_POH' class='input-helper input-helper--checkbox'>POH</label> 
                          <input type="checkbox" id="inc_PMH" name="inc_PMH"><label for='inc_PMH' class='input-helper input-helper--checkbox'>PMH</label>
                        </div>
                      </dt>  
                        <dd id="IMP_start_acc" name="IMP_start_acc">
                          ...
                          if ($total_DX == '1') {
                            ...
                            //This is in development so comment out for now.
                      ...
                      *  If the provider-specific list does not exist,
                      *  create it and populate it with generics - choose items based on form's audience for default options.
                      */
                      $query = "select * from list_options where list_id like 'Provider_todo_".$_SESSION['authId']."' order by seq";
                      $TODO_data =sqlStatement($query);
                      $maxTODO = sqlFetchArray($TODO_data);
                      if (!$maxTODO) {  
                        ...
                        ('lists', ?, ?, ?, '1', '0', '', '', '')";
                        sqlStatement($query,array("Provider_todo_$providerID","Dr. $prov_data[lname] Orders",$seq));
                        $query = "INSERT INTO `openemr`.`list_options` (`list_id`, `option_id`, `title`, `seq`, `is_default`, `option_value`, `mapping`, `notes`, `codes`) VALUES
                          ('Provider_todo_".$providerID."', 'VF', 'Visual Field (92083)', '1', '0', '0', '', '', '92083'),
                          ('Provider_todo_".$providerID."', 'ExtPhoto', 'External Photos', '2', '0', '0', '', '', '92285'),
                          ('Provider_todo_".$providerID."', 'OCTDisc', 'OCT Disc (92133)', '3', '0', '0', '', '', '92133'),
                          ('Provider_todo_".$providerID."', 'OCTRetina', 'OCT Retina (92134)', '4', '0', '0', '', '', '92134'),
                          ('Provider_todo_".$providerID."', 'FundusPhoto', 'Retina Photo (92250)', '5', '0', '0', '', '', '92250'),
                          ('Provider_todo_".$providerID."', 'Topo', 'Corneal Topography (92025)', '6', '0', '0', '', '', '92025'),
                          ('Provider_todo_".$providerID."', 'Gonio', 'Gonio (92020)', '7', '0', '0', '', '', '92020'),
                          ('Provider_todo_".$providerID."', 'Pachy', 'Pachymetry (76514)', '8', '0', '0', '', '', '76514')";
                          sqlStatement($query);
                        $query = "select * from list_options where list_id =? order by seq";
                        $TODO_data =sqlStatement($query,array("Provider_todo_$providerID"));
                      }
                        ?>
                      <dt class="borderShadow">
                          <!--
                          <i title="Build your plan." class="fa fa-share-square-o fa-flip-horizontal" id="make_new_PLAN" name="make_new_PLAN"></i>
                          -->  <span><?php echo xlt('PLAN: Next Visit Orders'); ?></span> 
                            <a href="/openemr/interface/super/edit_list.php?list_id=Provider_todo_<?php echo $_SESSION['authId']; ?>" target="RTop" 
                      ...
                      <dd>
                          <table>
                          <tr class="" style="vertical-align:bottom;margin:10;">
                              <td></td>
                          </tr>
                          <tr>
                            <td style="padding-right:20px;padding-left:20px;">
                              ...
                              /* 
                               * TODO items:  
                               * We are going to iterate through this provider's PLAN list to display it on the form.
                               * We need to pass this list to javascript as a variable (with all the info from the table)
                               * to build the plan.  
                               * Javascript will enter the text for all chosen selections under the Next Visit Orders: header
                               * both in the report and on the next visit...
                               * This ensures the compliance police are happy too.
                               * Alternatively, we wish to be able to drag each of these and drop them into a PLAN in the  
                               * Imp/Plan list. 
                               */
                              $query = "select * from list_options where list_id like 'provider_todo_$_SESSION[authId]' order by seq";
                              $TODO_data =sqlStatement($query);
                              ...
                              while ($row = sqlFetchArray($TODO_data)) {
                                ...
                                echo "<input type='checkbox' id='PLAN$counter' name='PLAN[]' $checked value='$row[title]'> ";
                                $label = text(substr($row['title'],0,30));
                                echo "<label for='PLAN$counter' class='input-helper input-helper--checkbox'>";
                                echo $label."</label><br />";
                                $count++;$counter++;
                                if ($count =="2") {
                    ...
                    <!--      <tr>
                            <td colspan="2" style="padding-right:20px;padding-left:20px;">
                              <?php
                                if (in_array('PLAN2',$PLAN_arr)) {
                                  echo "here there is a plan2 in ".$PLAN_arr;
                                  $plan2 = explode("PLAN2:",$PLAN_arr);
                                }
                              ?>
                              <textarea type="text" id="PLAN2" style="width:100%;margin:2 auto;"><?php echo text($plan2[1]); ?></textarea>
                            </td>
                          </tr>
                        -->
                          </table>
                         ...
                         *  This is the placeholder for that functionality.
                         *  Dx. codes in the Imp/Plan list make up the ICD-10 list.
                         *  If a DX CODE is missing, it can't show up in a bill...
                         *  If a DX CODE is present, we need a way to delete it from the bill, reorder them, attach them
                         *  to the visit code as a billing module requires, etc.
                         *  Perhaps a reformatted version of the billing module right here?
                         *  We need a way to select the visit code for this encounter.  There is a rudiment for this already below.
                         ...
                         */
                        ?>
                      <dt class="borderShadow"><span>Coding Hints</span></dt>
                      <dd>
                          <div style="padding:20px;">
                          <span class="CODE_LOW"><i class="fa fa-check"></i> <?php echo xlt('Limited HPI documented.  To reach "Detailed" documentation, you should document the status of three chronic or inactive problems, or at least 4 elements of the present illness.'); ?><br /></span>
                          <span class="CODE_HIGH nodisplay"><i class="fa fa-check"></i> <?php echo xlt('Detailed HPI documented.'); ?><br /></span>
                          <span class="DIL_RISKS nodisplay"><i class="fa fa-check"></i> <?php echo xlt('It appears you have enough documentation to bill a Level 4 exam.'); ?><br /></span>
                          <span id="neurosens_code" name="neurosens_code" class="nodisplay"><i class="fa fa-check"></i> NeuroSensory Exam (92060) Criteria.</span>
                          </div>
                      </dd>
                      </dl>
                    </div>
    ...
    <script type="text/javascript" src="<?php echo $GLOBALS['webroot']; ?>/interface/forms/<?php echo $form_folder; ?>/js/eye_base.js"></script>
    ...
    <div id="right-panel" name="right-panel" class="panel_side">
      <div style="margin-top:20px;text-align:center;font-size:1.2em;">
        ...
        <span class="fa fa-user-md fa-sm" style="margin:2px;" name="Shorthand_kb" title="<?php echo xla("Open the Shorthand Window and display Shorthand Codes"); ?>"></span>
        <span class="fa fa-close" id="close-panel-bt" style="margin:2px;"></span>
      </div>
      <div id="right_panel_refresh" name="right_panel_refresh">
        <?php 
        ...
        // The function display_PRIOR_section("PMSFH",$id,$id,$pid) in php/eye_mag_functions.php 
        // shows the PMSFH to the right of the PMH zone in the QP zone.  
        // The function show_PMSFH_panel($PMSFH) creates the right panel NavBar from this array...
