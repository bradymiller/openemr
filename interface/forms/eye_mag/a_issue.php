...
//specifically to build $PMSFH
  ...
  $encounter = 0 + (empty($_REQUEST['encounter']) ? 0 : $_REQUEST['encounter']);

  // A nonempty thistype is to assign an issue type for a new issue. 
...
// NOW we only get here on starting the form.  Saves and deletes go via ajax,
// so just build it with default to POH, using any pre-existing PMSFH data.
// The only data we need to know for this is SOCH,FH and ROS as these require
// a different mechanism for entering data.
// Maybe we can list the PMSFH for the section here also????  
// Yes we can to the right of the pick list...
 ...
 // of their frequency.  Start at 10 should they want to always have something at the top.
  ...
  foreach ($PMSFH[0] as $key => $value) {
    ...
    if ($key =="PMH") { // "0" = medical_problem_issue_list leave out Dental "4"
      ...
      //have to make sure a Tech sees what the provider prefers.  Is $_SESSION[authID] going to work here?
      if (sqlNumRows($qry) < '4') { //if they are just starting out, use the list_options for all
        $qry = sqlStatement("SELECT * FROM list_options WHERE list_id = ? and subtype not like 'eye'",array('medical_problem'."_issue_list"));
      }
      ...
      if (sqlNumRows($qry) < '4') { //if they are just starting out, use the list_options for all
        $qry = sqlStatement("SELECT * FROM list_options WHERE list_id = ? and subtype not like 'eye'",array('medication'."_issue_list"));
      }
    } elseif ($key =="Surgery") { 
      $qry = sqlStatement("SELECT title, title as option_id, count(title) AS freq  FROM `lists` WHERE `type` LIKE ? and 
        subtype = '' and pid in (select pid from form_encounter where provider_id =? 
      ...
      if (sqlNumRows($qry) < '4') { //if they are just starting out, use the list_options for all
        $qry = sqlStatement("SELECT * FROM list_options WHERE list_id = ? and subtype not like 'eye'",array('surgery'."_issue_list"));
      }
    } elseif ($key =="Allergy") { 
      $qry = sqlStatement("SELECT title, title as option_id, count(title) AS freq  FROM `lists` WHERE `type` LIKE ? and 
        subtype = '' and pid in (select pid from form_encounter where provider_id =? 
      ...
      if (sqlNumRows($qry) < '4') { //if they are just starting out, use the list_options for all
        $qry = sqlStatement("SELECT * FROM list_options WHERE list_id = ? and subtype not like 'eye'",array('allergy'."_issue_list"));
      }
    } elseif ($key == "POH") { // POH medical group - leave POsurgicalH for now. 
    ...
    if ($local =="1") { // leave FH/SocHx/ROS for later - done below separately
      while($res = sqlFetchArray($qry)){
        echo " aopts['$key'][aopts['$key'].length] = new Option('".attr(trim($res['option_id']))."', '".attr(xl_list_label(trim($res['title'])))."', false, false);\n";
      }
    }
    ++$i;
  }
...
// newtype('POH'); //start showing POH please
function newtype(index) {
  ...
  } else if (index == 'POH') { //POH
    document.getElementById('title_diagnosis'   ).textContent ="<?php echo xlt('Eye Dx').":"; ?>";
    document.getElementById('row_diagnosis'     ).style.display = '';
    document.getElementById('row_begindate'     ).style.display = '';
    document.getElementById('row_referredby'    ).style.display = '';
...
function deleteme() {
  ...
  $.ajax({
           ...
           data    : { 
                        ...
                        issue  : f.issue.value,
                        deletion : '1',
                        PMSFH  : '1'   //will send the obj back to build the panels.
                      },
            url    : url      // the url where we want to POST
           }).done(function (result){
            // CLEAR THE FORM TOO...
    ...
    action='a_issue.php?pid=<?php echo attr($pid); ?>&encounter=<?php echo attr($encounter); ?>'
     ...
     <input type="hidden" name="uniqueID" id="uniqueID" value = "<?php echo attr($uniqueID); ?>">
        ...
        foreach ($PMSFH[0] as $key => $value) {   
          ...
          $HELLO[$key] = '<input type="radio" name="form_type" id="PMSFH_'.$key.'" value="'.$key.'" '.$checked.' onclick="top.restoreSession();newtype(\''.$key.'\');" /><span style="margin-top:2px;font-weight:bold;">'.
                          '<label class="input-helper input-helper--checkbox" for="PMSFH_'.$key.'" style="font-size:0.7em;">' . $key_short_title . '</label></span>&nbsp;';
         // ++$index;
        }
        //put them in the desired display order  
        echo $HELLO['POH'].$HELLO['PMH'].$HELLO['Medication'].$HELLO['Surgery'].$HELLO['Allergy'].$HELLO['FH'].$HELLO['SOCH'].$HELLO['ROS'];
        ?>
    <div class="borderShadow" style="text-align:left;margin-top:7px;width;98%;">
      <table  border='0' width='95%'>
        <tr id='row_quick_picks'>
          ...
          <td valign='top'  colspan="3">
            ...
            This "size" variable below determines how many items appear in the select list.  ==5 fits panel in all browsers.
            ==5 is actually small and more would be useful.  Consider making two rows to show 10 options, or even 3 rows?
            Or move the save button into the panel for more space.
            I don't know about athletic teams == 10 since this is borrowed code from the base openEMR, but I left it so
            others will know where this all came from and to fiddle with the layout of their forms.  
            Answer for now: set it to == 5
            */
            ?>
        ...
        <tr id="row_title">
          ...
          <td  colspan="3">
            <input type='text' size='40' name='form_title' id='form_title' value='<?php echo xla($irow['title']) ?>' style='width:100%;text-align:left;' />
          </td>
        </tr>
        <tr id="row_diagnosis">
        ...
        <tr id='row_begindate'>
         ...
         <tr id='row_occurrence'>
          ...
          <td colspan="3">
           ...
           ?><a href="/openemr/interface/super/edit_list.php?list_id=occurrence" target="RTop" 
              title="<?php echo xla('Click here to Edit the Course/Occurrence List'); ?>" style="color:black;"><i class="fa fa-pencil fa-fw"></i></a></i>
          </td>
         </tr>
        ...
        <tr id='row_comments'>
          ...
          <td colspan="3">
           <textarea name='form_comments' id='form_comments' rows='2' cols='40' wrap='virtual' style='width:100%'><?php echo text($irow['comments']) ?></textarea>
          </td>
        </tr>
        <tr id="row_outcome">
      ...
      <table id="row_social" width="100%">      
            ...
            <tbody>
                ...
                <tr>
                  <td class="right"  nowrap>Tobacco:</td>
                  <td class="text data" colspan="3">
                    <table cellpadding="0" cellspacing="0">
                      <tr>
                        ...
                        <td><input name="radio_tobacco" type="radio" id="radio_tobacco[not_applicable]" value="not_applicabletobacco" onclick="smoking_statusClicked(this)">
                ...
                <tr>
                  <td  class="right" nowrap>Coffee:</td>
                  <td class="text data" colspan="3">
                    <table cellpadding="0" cellspacing="0">
                  <tbody>
                    <tr>
                      ...
                      <td><input name="radio_coffee" type="radio" id="radio_coffee[not_applicable]" value="not_applicablecoffee" onclick="coffee_statusClicked(this)">
                        <i class="fa fa-history"></i>
                        </td>
                    </tr>
                  </tbody>
                ...
                <tr>
                  <td class="right"  nowrap>Alcohol:</td>
                  <td class="text data" colspan="3">
                    <table cellpadding="0" cellspacing="0">
                      <tbody>
                        <tr><td><input type="text" name="form_alcohol" id="form_box" size="20" value="<?php echo $result2['alcohol']['resnote']; ?>">&nbsp;</td><td class="bold">&nbsp;&nbsp;</td><td class="text"><input type="radio" name="radio_alcohol" id="radio_alcohol[current]" value="currentalcohol" <?php if ($PMSFH[0]['SOCH']['alcohol']['restype'] =='currentalcohol') echo "checked"; ?>>Current&nbsp;</td>
                          ...
                          <td><input name="radio_alcohol" type="radio" id="radio_alcohol[not_applicable]" value="not_applicablealcohol" onclick="alcohol_statusClicked(this)">
                        <i class="fa fa-history"></i>
                        </td>
                        </tr>
                      </tbody>
                    ...
                    <table cellpadding="0" cellspacing="0">
                      <tbody>
                        <tr>
                          ...
                          <td><input name="radio_recreational_drugs" type="radio" id="radio_recreational_drugs[not_applicable]" value="not_applicablerecreational_drugs" onclick="recreational_drugs_statusClicked(this)">
                        <i class="fa fa-history"></i>
                      </td>
                        </tr>
                      </tbody>
                ...
                <tr class="nodisplay" ><td class="right"  nowrap>Counseling:</td><td class="text data" colspan="3"><table cellpadding="0" cellspacing="0"><tbody><tr><td><input type="text" name="form_counseling" id="form_box" size="20" value="<?php echo $result2['counseling']['resnote']; ?>">&nbsp;</td><td class="bold">&nbsp;&nbsp;</td><td class="text"><input type="radio" name="radio_counseling" id="radio_counseling[current]" value="currentcounseling" <?php if ($PMSFH[0]['SOCH']['counseling']['restype'] =='currentcounseling') echo "checked"; ?>>Current&nbsp;</td>
                  ...
                  <td><input name="radio_counseling" type="radio" id="radio_counseling[not_applicable]" value="not_applicablecounseling" onclick="counseling_statusClicked(this)">
                        <i class="fa fa-history"></i>
                  </td>
                </tr></tbody></table></td></tr>
            
                <tr><td class="right" nowrap>Exercise:</td><td class="text data" colspan="3"><table cellpadding="0" cellspacing="0"><tbody><tr><td><input type="text" name="form_exercise_patterns" id="form_box" size="20" value="<?php echo $result2['exercise_patterns']['resnote']; ?>">&nbsp;</td><td class="bold">&nbsp;&nbsp;</td><td class="text"><input type="radio" name="radio_exercise_patterns" id="radio_exercise_patterns[current]" value="currentexercise_patterns" <?php if ($PMSFH[0]['SOCH']['exercise_patterns']['restype'] =='currentexercise_patterns') echo "checked"; ?>>Current&nbsp;</td>
                  ...
                  <td><input name="radio_exercise_patterns" type="radio" id="radio_exercise_patterns[not_applicable]" value="not_applicableexercise_patterns" onclick="exercise_patterns_statusClicked(this)">
                        <i class="fa fa-history"></i>
                  </td>
                </tr></tbody></table></td></tr>
                
                <tr class="nodisplay"><td class="right"  nowrap>Hazardous Activities:</td><td class="text data" colspan="3"><table cellpadding="0" cellspacing="0"><tbody><tr><td><input type="text" name="form_hazardous_activities" id="form_box" size="20" value="<?php echo $result2['hazardous_activities']['resnote']; ?>">&nbsp;</td><td class="bold">&nbsp;&nbsp;</td><td class="text"><input type="radio" name="radio_hazardous_activities" id="radio_hazardous_activities[current]" value="currenthazardous_activities" <?php if ($PMSFH[0]['SOCH']['hazardous_activities']['restype'] =='currenthazardous_activities') echo "checked"; ?>>Current&nbsp;</td>
                  ...
                  <td><input name="radio_hazardous_activities" type="radio" id="radio_hazardous_activities[not_applicable]" value="not_applicablehazardous_activities" onclick="hazardous_activities_statusClicked(this)">
                        <i class="fa fa-history"></i>
                  </td>
                </tr></tbody></table></td></tr>
               
                <tr><td class="right"  nowrap>Sleep:</td><td class="text data" colspan="3"><input type="text" name="form_sleep_patterns" id="form_box" size="20" title="Sleep patterns" value="<?php echo $result2['sleep_patterns']['resnote']; ?>"></td></tr>
                
                <tr class="nodisplay">
                  <td class="right"  nowrap>Seatbelt:</td>
                  <td class="text data" colspan="3">
      ...
      <table id="row_FH" name="row_FH" width="90%">
        <tr>
          <td class="right" nowrap>Glaucoma:</td>
          ...
          <td class="right" nowrap>Cataract:</td>
        ...
        <tr>
          <td class="right" nowrap>Blindness:</td>
          ...
          <td class="right" nowrap>Amblyopia:</td>
        ...
        <tr>
          <td class="right" nowrap>Strabismus:</td>
          ...
          <td class="right" nowrap>Epilepsy:</td>
          <td class="text data">
        ...
        <tr>
          <td class="right" nowrap>Cancer:</td>
          <td class="text data">
          ...
          <td class="right" nowrap>Diabetes:</td>
          <td class="text data">
        ...
        <tr>
          ...
          <td class="right" nowrap>Heart Problems:</td>
          <td class="text data">
        ...
        <tr>
          <td class="right" nowrap>Stroke:</td>
          <td class="text data">
          ...
          <td class="right" nowrap>Other:</td>
      ...
      <table id="row_ROS" name="row_ROS" width="95%" class="ROS_class">
        <tr>
          <td>
           &nbsp;
          </td>
        ...
        <tr>
          <td class="right" nowrap>General:</td>
          <td>
          ...
          <td class="right" nowrap>HEENT:</td>
          <td>
        ...
        <tr>
          ...
          <td class="right" nowrap>Pulmonary:</td>
          <td>
    ...
    <center>
      <p style="margin-top:10px;">
        ...
        <?php
          ...
          ?>
        &nbsp;
        <input type='button' class='<?php echo $display_delete; ?>' name='delete_button' id='delete_button' onclick='top.restoreSession();deleteme();' value='<?php echo xla('Delete'); ?>' />
        &nbsp;
        <input type='button' name='cancel_button' id='cancel_button' value='<?php echo xla('Cancel'); ?>' onclick='clearme();' />
        </p>
      </center>
    </form>
  </div>
