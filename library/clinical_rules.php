...
function allergy_conflict($patient_id,$mode,$user,$test=FALSE) {
  ...
  // Collect allergies
  $res_allergies = sqlStatement("SELECT `title` FROM `lists` WHERE `type`='allergy' AND `activity`=1 AND `pid`=?", array($patient_id));
  $allergies = array();
  ...
  if (!empty($sqlParam)) {
    ...
    $res_meds = sqlStatement("SELECT `title` FROM `lists` WHERE `type`='medication' " .
                             "AND `activity`=1 AND `title` IN (" . $sqlIN . ") AND `pid`=?", array($sqlParam));
    ...
    $res_rx = sqlStatement("SELECT `drug` FROM `prescriptions` WHERE `active`=1 " .
                           "AND `drug` IN (" . $sqlIN . ") AND `patient_id`=?", array($sqlParam));
  ...
  if ( (!empty($conflicts_unique)) && $GLOBALS['enable_alert_log'] && ($test===FALSE) ) {
    $new_conflicts = compare_log_alerts($patient_id,$conflicts_unique,'allergy_alert',$_SESSION['authId'])
  }
